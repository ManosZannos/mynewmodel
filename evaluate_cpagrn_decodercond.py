"""
evaluate_cpagrn_decodercond.py — Evaluation for CPA-GRN, Decoder Conditioning variant.

Same ADE/FDE reporting format as evaluate_cpagrn.py, but loads
CPAGRNDecoderCond and reads top_k/cond_dim/gru_layers from the checkpoint's
saved args (NOT from CLI — same convention as top_k in evaluate_cpagrn.py).

Usage:
    python evaluate_cpagrn_decodercond.py --tag CPAGRN_v5_decodercond_gru2_lr5e4_obs10_pred10_s42 \
...
from model_cpagrn_decodercond import CPAGRNDecoderCond


def get_args():
    p = argparse.ArgumentParser()
    p.add_argument('--tag',            type=str,   default='CPAGRN_decodercond_obs10_pred10_s42')
...
def main():
    ...
    print(f"top_k={saved.get('top_k', 10)}  gru_layers={saved.get('gru_layers', 2)}  "
          f"cond_dim={saved.get('cond_dim', 16)}")
    ...
    model = CPAGRNDecoderCond(
        ...
        cond_dim     = saved.get('cond_dim',   16),
    ).to(device)
    ...
    print(f'\n{"="*55}')
    print(f'  CPA-GRN (decoder cond) | {args.tag} | {args.split}')
    print('='*55)
