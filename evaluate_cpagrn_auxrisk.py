"""
evaluate_cpagrn_auxrisk.py — Evaluation for CPA-GRN, Auxiliary Risk Head variant.

Same ADE/FDE reporting format as evaluate_cpagrn.py. The aux head runs but
its output is ignored here — only the main displacement task is scored,
consistent with "zero added inference cost / dropped at deployment".

Usage:
    python evaluate_cpagrn_auxrisk.py --tag CPAGRN_v5_auxrisk_gru2_lr5e4_obs10_pred10_s42 \
...
from model_cpagrn_auxrisk import CPAGRNAuxRisk


def get_args():
    p = argparse.ArgumentParser()
    p.add_argument('--tag',            type=str,   default='CPAGRN_auxrisk_obs10_pred10_s42')
...
def main():
    ...
    print(f'Loaded epoch {ckpt["epoch"]}  val_loss(main)={ckpt.get("val_loss","?"):.6f}')
    print(f"top_k={saved.get('top_k', 10)}  gru_layers={saved.get('gru_layers', 2)}  "
          f"aux_weight={saved.get('aux_weight', 0.1)}")
    ...
    with torch.no_grad():
        for obs, pred_gt, mask, _ in loader:
            ...
            pred_disp, _aux_pred = model(obs, mask=mask, stats=stats)  # aux ignored
    ...
    print(f'\n{"="*55}')
    print(f'  CPA-GRN (aux risk head) | {args.tag} | {args.split}')
    print('='*55)
