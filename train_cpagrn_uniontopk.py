"""
train_cpagrn_uniontopk.py — Training script for CPA-GRN, Union Top-K variant.

Neighbor selection = top_k_dist NEAREST by distance UNION top_k_risk LOWEST
DCPA among TCPA>0 (approaching) neighbors. See model_cpagrn_uniontopk.py for
full rationale and implementation.

Usage:
    python train_cpagrn_uniontopk.py --obs_len 10 --pred_len 10 --gru_layers 2 \
        --top_k_dist 12 --top_k_risk 3 --seed 42 \
        --tag CPAGRN_v5_uniontopk_gru2_obs10_pred10_s42 --gpu_num <GPU>
"""
...
def get_args():
    ...
    p.add_argument('--tag',            type=str,   default='CPAGRN_uniontopk_obs5_pred5')
...
def main():
    ...
    log.info(f'Model: model_cpagrn_uniontopk.py (top_k_dist={args.top_k_dist} '
              f'UNION top_k_risk={args.top_k_risk}, gru_layers={args.gru_layers})')
    ...
    model = CPAGRNUnionTopK(
        ...
        pred_len     = args.pred_len,
        top_k_dist   = args.top_k_dist,
        top_k_risk   = args.top_k_risk,
    ).to(device)
