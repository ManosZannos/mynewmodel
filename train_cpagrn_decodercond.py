"""
train_cpagrn_decodercond.py — Training script for CPA-GRN, Decoder Conditioning variant.

Same encoder/GRU/neighbor-selection as gru2 (headline). Only the decoder
differs: per-step conditioning on (TCPA, DCPA, k/pred_len) to the closest
approaching neighbor. See model_cpagrn_decodercond.py for full rationale.

Usage:
    python train_cpagrn_decodercond.py --obs_len 10 --pred_len 10 --gru_layers 2 \
        --top_k 10 --cond_dim 16 --seed 42 --lr 5e-4 \
        --tag CPAGRN_v5_decodercond_gru2_lr5e4_obs10_pred10_s42 --gpu_num <GPU>
"""
...
def get_args():
    ...
    p.add_argument('--tag',            type=str,   default='CPAGRN_decodercond_obs10_pred10')
...
def main():
    ...
    log.info(f'Model: model_cpagrn_decodercond.py (gru_layers={args.gru_layers}, '
              f'top_k={args.top_k}, cond_dim={args.cond_dim})')
